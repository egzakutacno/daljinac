...
func New(apiKey, version, serverURL, machineID string) *Server {
...
func (s *Server) SetServerURL(url string) {
	s.info.ServerURL = url
}
...
func (s *Server) handleRoot(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]string{"agent": "daljinac", "status": s.Status().String()})
}
...
func (s *Server) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	jsonResp(w, 200, map[string]interface{}{"ok": true, "size": len(data)})
}
...
func (s *Server) handleFiles(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range entries {
		...
		list = append(list, Entry{Name: e.Name(), IsDir: e.IsDir(), Size: size, ModTime: modTime})
	}
	jsonResp(w, 200, list)
}
